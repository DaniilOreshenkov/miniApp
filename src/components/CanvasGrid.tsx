  ...
  ({
    ...
    const getExportFiles = useCallback((
      ...
      try {
        ...
        // dataURL храним для надёжного fallback download
        ...
        const gridBlob = new Blob([gridBytesWithMeta.buffer as ArrayBuffer], { type: "image/png" });
        ...
        if (colorsDataURL) {
          files.push(new File([dataURLToBlob(colorsDataURL)], `${exportName}_colors.png`, { type: "image/png" }));
          dataURLs.push(colorsDataURL);
        }

        return { files, dataURLs };
      } catch (err) {
        console.error("[getExportFiles] failed:", err);
        return null;
      }
        ...
        for (let index = placedShapes.length - 1; index >= 0; index -= 1) {
          ...
          const nextPlacedShapes = (() => {
            ...
            return [
              ...withoutSelected,
              {
                ...
                fillMode: previousActiveShapeFillMode,
          ...
          syncShapeLayersToParent([
            ...nextPlacedShapes,
            {
              ...
              fillMode: placedShape.fillMode ?? "fill",
