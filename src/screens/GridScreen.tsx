...
if (
  ...
  `;
  document.head.appimport React, { useEffect, useMemo, useRef, useState } from "react";
...
const pageStyle: React.CSSProperties = {
  ...
  maxHeight: "var(--tg-viewport-stable-height, var(--app-height, 100vh))",
  background:
    "radial-gradient(circle at top, rgba(68,110,255,0.22) 0%, rgba(68,110,255,0) 34%), linear-gradient(180deg, #101827 0%, #0b1220 100%)",
  position: "relative",
  overflow: "hidden",
  overscrollBehavior: "none",
};
...
const contentWrapStyle: React.CSSProperties = {
  minHeight: "100%",
  height: "100%",
  padding: "max(72px, calc(env(safe-area-inset-top) + 28px)) 14px max(28px, calc(env(safe-area-inset-bottom) + 16px))",
  display: "flex",
  flexDirection: "column",
  alignItems: "center",
  boxSizing: "border-box",
...
const bottomToolButtonStyle = (active: boolean): React.CSSProperties => ({
  ...
  background: active
    ? "linear-gradient(180deg, #4d79ff 0%, #315dff 100%)"
    : "rgba(255,255,255,0.06)",
  color: "#ffffff",
  cursor: "pointer",
  fontSize: 24,
