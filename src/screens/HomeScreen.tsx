...
const HomeScreen: React.FC<Props> = ({ onCreateGrid }) => {
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => setTabContentVisible(true));
    });
  }, [activeTab]);
  ...
  const homeContent = (
    <>
      <section ref={stickyRef} style={stickyHeroWrapStyle}>
        ...
        <button ref={buttonRef} onClick={openCreateSheet} style={primaryButtonStyle}>
          + Создать сетку
        </button>
      </section>
      ...
      {hasProjects && (
        <section style={sectionStyle}>
          <div style={sectionHeaderRowStyle}>
            ...
            <button style={ghostButtonStyle} onClick={() => setActiveTab("projects")}>
              Все
            </button>
          </div>
          ...
          <div style={projectsListStyle}>
            {latestProjects.map((project) => (
              <ProjectCard key={project.id} project={project} onClick={onCreateGrid} />
            ))}
          </div>
        </section>
      )}
  ...
  const content = useMemo(() => {
    ...
    return <ProjectsScreen projects={mockProjects} onProjectClick={onCreateGrid} />;
  }, [activeTab]);

  return (
    <div style={rootStyle}>
      <div ref={scrollContainerRef} style={scrollAreaStyle} onScroll={handleScroll}>
        <main style={{ ...mainStyle, opacity: tabContentVisible ? 1 : 0 }}>
          {content}
        </main>
      </div>
      ...
      <CreateProjectSheet
        ...
        onGridWidthChange={(v) => setGridWidth(sanitizeNumericInput(v))}
        onGridHeightChange={(v) => setGridHeight(sanitizeNumericInput(v))}
        onGridWidthBlur={() => setGridWidth((p) => clampGridValueOnBlur(p))}
        onGridHeightBlur={() => setGridHeight((p) => clampGridValueOnBlur(p))}
      />
    </div>
  );
};
